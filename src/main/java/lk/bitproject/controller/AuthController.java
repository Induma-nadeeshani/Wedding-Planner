...
public class AuthController {
    ...
    public static HashMap<String,Boolean> getPrivilages(User user, String module) {
        ...
        if (user.getUserName().equals("admin") ||user.getUserName().equals("owner")) {
            ...
            try {
                ...
                String query2 = "select  bit_or(sel) sel, bit_or(ins) ins, bit_or(upd) upd, bit_or(del) del from privilage where roles_role_id in (select role_id from user_role where user_id=(select user_id from users where username='" + user.getUserName() + "')) and module_id=(select id from module where name='" + module + "')";
    ...
    public static void setConnection() throws Exception{
        if (connection == null) {
            ...
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/goldencrown", "root", "1234");
        }
