function getDataFromObjectList(model, path) {
    var parts = path.split('.');
    if (parts.length > 1 && typeof model[parts[0]] === 'object') {
        return getDataFromObjectList(model[parts[0]], parts.splice(1).join('.'));
    } else {
...
function changeTab(viewname) {
    if(viewname=='divevent'){
        liformevent.classList.add('active');
        liformservice.classList.remove('active');
        liformreservation.classList.remove('active');
        litableview.classList.remove('active');
        divevent.style.display = "block";
        divservice.style.display = "none";
        divreservation.style.display = "none";
        divtable.style.display = "none";
    }
    if(viewname=='divservice'){
        liformevent.classList.remove('active');
        liformservice.classList.add('active');
        liformreservation.classList.remove('active');
        litableview.classList.remove('active');
    ...
    if(viewname=='divreservation'){
        liformevent.classList.remove('active');
        liformservice.classList.remove('active');
        liformreservation.classList.add('active');
        litableview.classList.remove('active');
        divevent.style.display = "none";
        divservice.style.display = "none";
    ...
    if(viewname=='divtable'){
        liformevent.classList.remove('active');
        liformservice.classList.remove('active');
        liformreservation.classList.remove('active');
        litableview.classList.add('active');
        divevent.style.display = "none";
        divservice.style.display = "none";
        divreservation.style.display = "none";
...
//set decimal points (rs.)
function  toDeimal(number,dpoint = 2) {
    return parseFloat(number).toFixed(dpoint);
...
function startWaiting(msg='Loading'){
    ...
    image.src='resourse/image/loading.gif';
...
function fillTable(tableid,obs,modifyFunction=null,deletefunction=null,printfunction=null){
    ...
    for(var i=0; i<obs.length; i++){
        ...
        if(hasButton){
            ...
            buttonup.setAttribute('rel', "tooltip");
            buttonup.setAttribute('title', "Update");
            buttonup.setAttribute('data-placement', "bottom");
            buttonup.classList.add('buttonup');
            buttonup.innerHTML = '<i class="fa fa-cogs"> </i>';
            buttonup.onclick=function(){
                //viewForm();
                // buttondel.disabled = "disabled";
                // buttonvie.disabled = "disabled";
            ...
            buttondel.setAttribute('rel', "tooltip");
            buttondel.setAttribute('title', "Delete row");
            buttondel.setAttribute('data-placement', "bottom");
            buttondel.classList.add('ml-2');
            buttondel.classList.add('mr-2');
            buttondel.innerHTML = '<i class="fa fa-recycle"></i>';
            ...
            buttonvie.setAttribute('data-toggle', "modal");
            buttonvie.setAttribute('data-target', "#exampleModalLong");
            buttonvie.classList.add('btn');
            buttonvie.classList.add('btn-primary');
            buttonvie.setAttribute('rel', "tooltip");
            buttonvie.setAttribute('title', "view row data");
            buttonvie.setAttribute('data-placement', "bottom");
...
function fillCombo3(combo, message, list, attribute,attribute1, selectedvalue) {
    combo.innerHTML = "";
    if (message != "") {
        var prompt = document.createElement("option");
        prompt.value = "";
    ...
    for (index in list) {
        var option = document.createElement("option");
        option.value = JSON.stringify(list[index]);
        var text = document.createTextNode(getDataFromObjectList(list[index],attribute)+ " - "+getDataFromObjectList(list[index],attribute1));
...
function fileChooser(parent,id,lblText,labelWidth,extenstions,maxsize,required,ob,prop,oldob){
    ...
    buttonArea.classList.add('col-md-12');
...
function setDefaultFile(id,file,type='image',name='',size='',static=false){
    ...
    if(type=='image'){
        if(file==null){
            image.src = 'resourse/image/noimage.png';
        }else{
            image.src = atob(file);
        }
    }else{
...
//report print
function printDiv(divid,width,height,headheight){
    ...
    newwindow.document.write("" +
        ...
        "</head>" +
        "<body><div style='margin-top:"+headheight+"px; '> <h1>Employee Details :</h1></div>" +
        "<div style='width: "+width+"px; height: "+height+"px;'>"+divid+"</div>" +
        "</body>" +
        "</html>");
